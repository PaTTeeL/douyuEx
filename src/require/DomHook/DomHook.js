class DomHook {
    ...
    constructor(elementOrSelector, isSubtree, callback, observeAttributes = false) {
        ...
        let instance = configMap.get(configKey);
        if (!instance) {
            const callbacks = new Map();
            const observer = new MutationObserver(mutations => {
                for (const [instance, callback] of callbacks) {
                    try {
                        callback.call(instance, mutations);
                    } catch (err) {
            ...
            try {
                ...
                configMap.set(configKey, instance);
        ...
        instance.callbacks.set(this, callback);
    ...
    closeHook() {
        ...
        if (configMap) {
            const instance = configMap.get(this._configKey);
            if (instance) {
                instance.callbacks.delete(this);
                if (instance.callbacks.size === 0) {
                    ...
                    //console.log(`DouyuEx DomHook: 移除节点回调，当前观察实例剩余回调: ${instance.callbacks.size}`, this._targetNode, this._configKey);
                }
            } else {
