...
function initPkg_ExpandTool_FullScreenPage_insertFunc() {
    ...
    gHotkey.add("t", () => {
        ...
        if (toggleContainer) {
            ...
            if (stageContainer) stageContainer.classList.toggle("large__pcRJn");
            //const playerContainer = stageContainer.getElementsByClassName("player__jsy1T")[0];
            //if (playerContainer) playerContainer.classList.toggle("hidden__Kqqgu");
            const fullScreenSendor = document.getElementsByClassName("fullScreenSendor-32a8a8")[0];
            if (fullScreenSendor) fullScreenSendor.classList.toggle("fullScreenSendorShow-fb7c32");
        ...
        if (loadData_ExpandTool("isHidePlayerSidebar")) {
            gDomObserver.waitForElement('.toggle__P8TKM').then(toggleContainer => {
                if (!toggleContainer.classList.contains("shrink__Sd0uK")) {
                    ...
                    //console.log("DouyuEx 自动折叠侧栏: 已是折叠状态，跳过点击脚本", toggleContainer);
                }
            });
        }
    });
