...
function initRouter_DouyuRoom_Popup() {
    // 画中画
    removeAD();
    gDomObserver.waitForElement('div.wfs-2a8e83').then(fullScreenPageButton => {
        fullScreenPageButton.click()
        const asidetoggleButton = document.querySelector('label.layout-Player-asidetoggleButton');
        if (asidetoggleButton) asidetoggleButton.click();
        const lastItem = document.querySelector('.tip-e3420a > ul > li:last-child');
